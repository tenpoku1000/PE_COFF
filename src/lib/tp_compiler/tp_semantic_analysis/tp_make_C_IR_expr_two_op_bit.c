...
// Convert parse tree to C IR(expressions: two operands: bit).

static bool make_C_IR_exclusive_OR_expression(
...
static bool make_C_IR_AND_expression(
    TP_SYMBOL_TABLE* symbol_table,
    TP_PARSE_TREE* parse_tree, TP_GRAMMER_CONTEXT grammer_context,
    TP_C_OBJECT* c_object, int32_t* func_arg_index, bool is_ignore_ABI,
    uint32_t* function_call_depth
    ...
    uint32_t* function_call_depth)
{
    ...
    case TP_PARSE_TREE_GRAMMER_INCLUSIVE_OR_EXPRESSION_2:{
        if (1 != element_num){
        ...
        if ( ! make_C_IR_exclusive_OR_expression(
    ...
    uint32_t* function_call_depth)
{
    ...
    case TP_PARSE_TREE_GRAMMER_EXCLUSIVE_OR_EXPRESSION_2:{
        if (1 != element_num){
    ...
    uint32_t* function_call_depth)
{
    ...
    case TP_PARSE_TREE_GRAMMER_AND_EXPRESSION_2:{
        if (1 != element_num){
        ...
        if ( ! tp_make_C_IR_equality_expression(
            symbol_table, parse_tree_child, grammer_context, c_object,
            func_arg_index, is_ignore_ABI, function_call_depth)){
