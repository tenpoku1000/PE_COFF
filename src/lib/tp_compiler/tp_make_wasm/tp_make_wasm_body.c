...
// Make WebAssembly with relocations.
    ...
    uint8_t* entry_point_symbol, bool is_origin_wasm)
{
    ...
    if (is_origin_wasm){
        ...
        tmp_section[0] = tp_make_section_type_UEFI(symbol_table);
        tmp_section[1] = tp_make_section_function_UEFI(symbol_table);
        tmp_section[5] = tp_make_section_export_UEFI(symbol_table);
    }else{

        // C compiler
        tmp_section[6] = tp_make_wasm_C(symbol_table, entry_point_symbol);
        tmp_section[0] = tp_make_section_type_C(symbol_table);
        tmp_section[1] = tp_make_section_function_C(symbol_table);
        tmp_section[5] = tp_make_section_export_C(symbol_table);
    }
    ...
    *section = tmp_section;
    ...
    if (tmp_section[0] && tmp_section[1] && tmp_section[2] &&
        tmp_section[3] && tmp_section[4] && tmp_section[5] && tmp_section[6]){
    ...
    uint32_t init_value, uint32_t* wasm_offset, uint32_t* coff_offset, uint32_t* symbol_index)
{
    if (symbol_table->member_is_add_data_section){
        ...
        if ( ! tp_append_coff_symbol(
            symbol_table, false/* is_external */, TP_SECTION_KIND_DATA, NULL, 0, 0, NULL)){
    ...
    *coff_offset = symbol_table->member_coff.member_data_pos;

    if ( ! tp_append_coff_symbol(
        symbol_table, is_external, TP_SECTION_KIND_DATA, symbol, symbol_length, *coff_offset, symbol_index)){
    ...
    uint32_t* wasm_offset, uint32_t* coff_offset, uint32_t* symbol_index)
{
    ...
    if (symbol_table->member_is_add_rdata_section){
        ...
        if ( ! tp_append_coff_symbol(
            symbol_table, is_external, TP_SECTION_KIND_RDATA, NULL, 0, 0, NULL)){
    ...
    *coff_offset = symbol_table->member_coff.member_rdata_pos;

    if ( ! tp_append_coff_symbol(
        symbol_table, is_external,
        TP_SECTION_KIND_RDATA, symbol, symbol_length, (uint32_t)*coff_offset, symbol_index)){
    ...
    uint8_t* symbol, rsize_t symbol_length, uint32_t* symbol_index)
{
    if (symbol_table->member_is_add_text_section){
        ...
        if ( ! tp_append_coff_symbol(
            symbol_table, false/* is_external */, TP_SECTION_KIND_TEXT, NULL, 0, 0, NULL)){
    ...
    if ( ! tp_append_coff_symbol(
        symbol_table, is_external, TP_SECTION_KIND_TEXT, symbol, symbol_length, 0, symbol_index)){
