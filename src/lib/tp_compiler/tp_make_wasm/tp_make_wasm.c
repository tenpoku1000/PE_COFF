...
static bool get_wasm_module_code_section(
    ...
    uint32_t section_num, uint32_t* code_section_index, uint32_t* return_type
);
static bool get_wasm_module_export_section_item_index(
    TP_SYMBOL_TABLE* symbol_table, TP_WASM_MODULE_SECTION** section,
    uint32_t section_num, uint8_t* name, uint8_t kind, uint32_t* item_index
);
static bool get_wasm_module_function_section_type(
    TP_SYMBOL_TABLE* symbol_table, TP_WASM_MODULE_SECTION** section,
    uint32_t section_num, uint32_t item_index, uint32_t* type
);
static bool get_wasm_module_type_section_return_type(
    TP_SYMBOL_TABLE* symbol_table, TP_WASM_MODULE_SECTION** section,
    uint32_t section_num, uint32_t type, uint32_t* return_type
);
static bool get_wasm_module_code_section_index(
    TP_SYMBOL_TABLE* symbol_table, TP_WASM_MODULE_SECTION** section,
    uint32_t section_num, uint32_t* code_section_index
);

bool tp_make_wasm(
    TP_SYMBOL_TABLE* symbol_table, uint8_t* entry_point_symbol, bool is_origin_wasm)
{
    if ( ! wasm_gen(symbol_table, entry_point_symbol, is_origin_wasm)){
    ...
    if ((false == symbol_table->member_is_no_output_files) ||
        ...
        symbol_table->member_is_output_wasm_text_file)){
    ...
    TP_SYMBOL_TABLE* symbol_table, uint8_t* entry_point_symbol, bool is_origin_wasm)
{
    ...
    if (symbol_table->member_is_int_calc_compiler){
        ...
        // C compiler
        if ( ! tp_make_wasm_coff(symbol_table, module, &section, entry_point_symbol, is_origin_wasm)){
    ...
    if (is_success){
...

        {
            ...
            memcpy(
                &(module->member_module_content->member_magic_number),
                TP_WASM_MODULE_MAGIC_NUMBER, sizeof(uint32_t)
            );
        ...
        if ((false == symbol_table->member_is_no_output_files) ||
            (symbol_table->member_is_no_output_files &&
            symbol_table->member_is_output_wasm_file)){
...
fail:
    ...
    if (section){
        ...
        for (size_t i = 0; module->member_section_num > i; ++i){
            ...
            if (section[i]){
                ...
                TP_FREE(
                    symbol_table,
                    &(section[i]->member_name_len_name_payload_data),
                    section[i]->member_section_size
                );
    ...
    if (module->member_module_content){
        ...
        TP_FREE(
            symbol_table, &(module->member_module_content),
            sizeof(TP_WASM_MODULE_CONTENT) + module->member_content_size
        );
    }
    ...
    TP_SYMBOL_TABLE* symbol_table, TP_WASM_MODULE* module)
{
    TP_WASM_MODULE_SECTION** tmp_section =
        (TP_WASM_MODULE_SECTION**)TP_CALLOC(
            symbol_table, module->member_section_num, sizeof(TP_WASM_MODULE_SECTION*)
        );
    ...
    TP_WASM_MODULE* module, TP_WASM_MODULE_SECTION*** section)
{
    ...
    for (uint32_t i = 0; module->member_section_num > i; ++i){
        ...
        if (section[i]){
            ...
            TP_FREE(
                symbol_table, &((*section)[i]->member_name_len_name_payload_data),
                (*section)[i]->member_section_size
            );
    ...
    uint32_t section_num, uint32_t* code_section_index, uint32_t* return_type)
{
    ...
    if ( ! get_wasm_module_function_section_type(
        symbol_table, section, section_num, item_index, &type)){
    ...
    if ( ! get_wasm_module_type_section_return_type(
        symbol_table, section, section_num, type, return_type)){
    ...
    if ( ! get_wasm_module_code_section_index(
        symbol_table, section, section_num, code_section_index)){
    ...
    uint32_t section_num, uint32_t item_index, uint32_t* type)
{
    ...
    uint32_t count = 0;
    TP_DECODE_UI32LEB128_GET_VALUE(symbol_table, function_section, payload, offset, count);

    for (uint32_t i = 0; count > i; ++i){

        uint32_t types = 0;
        TP_DECODE_UI32LEB128_GET_VALUE(symbol_table, function_section, payload, offset, types);
    ...
    uint32_t section_num, uint32_t type, uint32_t* return_type)
{
    ...
    for (uint32_t i = 0; type_count > i; ++i){
        ...
        TP_DECODE_UI32LEB128_GET_VALUE(symbol_table, type_section, payload, offset, ret_count);

        switch (ret_count){
        case 0:
            if (type == i){
                *return_type = TP_WASM_RETURN_VOID;
                return true;
            }
            break;
        case TP_WASM_MODULE_SECTION_TYPE_RETURN_COUNT:{
