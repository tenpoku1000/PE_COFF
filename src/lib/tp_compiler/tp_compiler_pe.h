...
#define IS_PE_PRINT(symbol_table) ( \
    (false == (symbol_table)->member_is_no_output_files) && \
    ((symbol_table)->member_is_output_pe_coff_file) \
)
...
#define TP_PE_8_BYTES_ALIGNMENT 8
#define TP_PE_16_BYTES_ALIGNMENT 16
...
#define TP_PE_PADDING_8_BYTES_ALIGNMENT(value) (rsize_t)(-((int64_t)value) & (TP_PE_8_BYTES_ALIGNMENT - 1))
#define TP_PE_PADDING_16_BYTES_ALIGNMENT(value) (rsize_t)(-((int64_t)value) & (TP_PE_16_BYTES_ALIGNMENT - 1))
...
typedef struct TP_PE_EXPORT_{
    uint32_t ExportFlags;
    uint32_t TimeDateStamp;
    uint16_t MajorVersion;
    uint16_t MinorVersion;
    uint32_t DllName;            // RVA
    uint32_t OrdinalBase;
    uint32_t NumberOfExportAddressTable;
    uint32_t NumberOfNames;
    uint32_t ExportAddressTable; // RVA
    uint32_t NamePointerTable;   // RVA
