...
#define TP_WASM_VAR_TYPE_PSEUDO_PREFIX 0xFF00
#define TP_WASM_VAR_TYPE_PSEUDO_I32 0xFF7f
#define TP_WASM_VAR_TYPE_PSEUDO_I64 0xFF7e
...
typedef enum TP_WASM_RELOCATION_TYPE_{
    ...
    TP_WASM_RELOCATION_TYPE_PROLOGUE,  // Prologue.
    TP_WASM_RELOCATION_TYPE_EPILOGUE,  // Epilogue.
    TP_WASM_RELOCATION_TYPE_SKIP    ,  // Skip code.
    TP_WASM_RELOCATION_TYPE_BASE,      // Transform bottom of linear memory to RBP of x64.
    TP_WASM_RELOCATION_TYPE_REL32,     // Base of load/store 32 bits offset to offset of COFF.
    TP_WASM_RELOCATION_TYPE_REL64,     // Base of load/store 64 bits offset to offset of COFF.
    TP_WASM_RELOCATION_TYPE_STRING_LITERAL32,  // 32 bits offset of string literal to offset of COFF.
...
#define TP_WASM_ARG_INDEX_VOID -2
#define TP_WASM_ARG_INDEX_EAX -1
...
#define TP_WASM_CODE_REL32(symbol_table, index, code, offset, symbol, symbol_length, symbol_index) \
\
    tmp_index_ = (code); \
\
    if ( ! tp_wasm_add_relocation(symbol_table, &(TP_WASM_RELOCATION){ \
        .member_type = TP_WASM_RELOCATION_TYPE_REL32, \
        .member_begin_index = (index) - symbol_table->member_wasm_relocation_begin, \
        .member_size = tmp_index_, \
        .member_offset32 = (offset), \
        .member_offset64 = 0, \
        .member_arg_index = TP_WASM_ARG_INDEX_VOID, \
        .member_type_return = TP_WASM_RETURN_VOID, \
\
...
#define TP_WASM_CODE_STRING_LITERAL32( \
    symbol_table, index, code, offset, symbol, symbol_length, symbol_index, arg_index, type_return) \
\
    tmp_index_ = (code); \
\
    if ( ! tp_wasm_add_relocation(symbol_table, &(TP_WASM_RELOCATION){ \
        .member_type = TP_WASM_RELOCATION_TYPE_STRING_LITERAL32, \
        .member_begin_index = (index) - symbol_table->member_wasm_relocation_begin, \
        .member_size = tmp_index_, \
        .member_offset32 = (offset), \
        .member_offset64 = 0, \
...
#define TP_WASM_CODE_ARG(symbol_table, index, code, arg_index, type_return) \
\
    tmp_index_ = (code); \
\
    if ( ! tp_wasm_add_relocation(symbol_table, &(TP_WASM_RELOCATION){ \
...
#define TP_WASM_CODE_CALL(symbol_table, index, code, type_return) \
\
    tmp_index_ = (code); \
\
    if ( ! tp_wasm_add_relocation(symbol_table, &(TP_WASM_RELOCATION){ \
...
typedef struct tp_wasm_type_function_{
    TP_WASM_TYPE member_type_return;
    uint32_t member_parameter_num;
    TP_WASM_TYPE member_type_parameter[];
}TP_WASM_TYPE_FUNCTION;
