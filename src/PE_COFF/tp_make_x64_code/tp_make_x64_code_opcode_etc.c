    ...
    if (x64_code_buffer){
        ...
        if (false == src->member_is_stack_base){
            ...
            bool is_text = (wasm_relocation && (wasm_relocation->member_symbol_table_index) &&
                ((TP_WASM_RELOCATION_TYPE_REL32 == wasm_relocation->member_type) ||
            ...
            bool is_rdata = (wasm_relocation && (wasm_relocation->member_symbol_table_index) &&
                ((TP_WASM_RELOCATION_TYPE_STRING_LITERAL32 == wasm_relocation->member_type) ||
    ...
    uint8_t* x64_code_buffer, uint32_t x64_code_offset, TP_WASM_STACK_ELEMENT* op1)
{
    ...
    if (x64_code_buffer){
        ...
        if ((false == is_op1_local_memory) && (false == is_op1_global_memory)){
            ...
            if (is_disp8){
                ...
                x64_code_buffer[x64_code_offset + x64_code_size] = (uint8_t)(op1->member_offset);

                ++x64_code_size;
            }else{

                memcpy(
                    &(x64_code_buffer[x64_code_offset + x64_code_size]), &(op1->member_offset),
                    sizeof(op1->member_offset)
                );
